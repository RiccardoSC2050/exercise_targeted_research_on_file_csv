...
def numero_totale_dipendenti(df) -> int:
    """Restituisce il numero totale di dipendenti."""
    return len(df)


def numero_dipendenti_per_istruzione(df) -> object:

    istruzione: str = input("per quale parametro vuoi il conteggio dei dipendenti? ")
    if istruzione in df.columns:
        return df[istruzione]
...
def media_anni_anzianita(df) -> float:
    ...
    return sum(anni_lavoro) / len(anni_lavoro)


if __name__ == "__main__":
    import pandas as pd

    from funzioni_su_fileCSV.letturaFile_csv import *
    ...
    print("Media anni di anzianità in azienda:", media_anni_anzianita(df))
